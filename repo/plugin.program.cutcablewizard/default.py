...
def install_build(url, name, version):
    ...
    try:
        ...
        # Create trigger files in HOME (root) for maximum reliability
        ...
        # FORCE CLOSE: This prevents Kodi from overwriting the build's skin settings
        xbmcgui.Dialog().ok("Success", "Build Applied! Kodi must now FORCE CLOSE to load the new skin.\n\nClick OK and then manually re-open Kodi.")
        os._exit(1) 
...
def main_menu():
    ...
    if choice == 0 and manifest:
        ...
        if smart_fresh_start():
            xbmcgui.Dialog().ok("Fresh Start", "Cleanup Complete. Force closing Kodi.")
            os._exit(1)
